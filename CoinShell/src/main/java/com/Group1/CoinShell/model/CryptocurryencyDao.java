...
public interface CryptocurryencyDao extends JpaRepository<Cryptocurrency,Integer>{
	...
	//語法 DECLARE @currencyName VARCHAR(10) ='BTC' SELECT symbol_of_cryptocurrency ,(SELECT USD_Price_of_Cryptocurrency FROM Cryptocurrency WHERE CONVERT(varchar,InformationDate,120) = (SELECT MAX(InformationDate) FROM Cryptocurrency WHERE CONVERT(varchar,InformationDate,120) like '%2022-05-13%' AND symbol_of_cryptocurrency=@currencyName) AND symbol_of_cryptocurrency=@currencyName) as 'TheFristUsdDate' ,(SELECT USD_Price_of_Cryptocurrency FROM Cryptocurrency WHERE CONVERT(varchar,InformationDate,120) = (SELECT MIN(InformationDate) FROM Cryptocurrency WHERE CONVERT(varchar,InformationDate,120) like '%2022-05-13%' AND symbol_of_cryptocurrency=@currencyName) AND symbol_of_cryptocurrency=@currencyName) as 'TheLastUsdDate' ,MAX(USD_Price_of_Cryptocurrency) AS 'TheHighestUsdPricePerUnit' ,MIN(USD_Price_of_Cryptocurrency)AS 'TheLowerUsdPricePerUnit' ,MAX(volume24hUsd)AS '24hUsdVolume' ,MAX(USD_Market_Cap)AS 'UsdMarkCap' ,(SELECT TWD_Price_of_Cryptocurrency FROM Cryptocurrency WHERE CONVERT(varchar,InformationDate,120) = (SELECT MAX(InformationDate) FROM Cryptocurrency WHERE CONVERT(varchar,InformationDate,120) like '%2022-05-13%' AND symbol_of_cryptocurrency=@currencyName) AND symbol_of_cryptocurrency=@currencyName) as 'TheFristTwdDate' ,(SELECT TWD_Price_of_Cryptocurrency FROM Cryptocurrency WHERE CONVERT(varchar,InformationDate,120) = (SELECT MIN(InformationDate) FROM Cryptocurrency WHERE CONVERT(varchar,InformationDate,120) like '%2022-05-13%' AND symbol_of_cryptocurrency=@currencyName) AND symbol_of_cryptocurrency=@currencyName) as 'TheLastTwdDate' ,MAX(TWD_Price_of_Cryptocurrency) AS 'TheHighestTwdPricePerUnit' ,MIN(TWD_Price_of_Cryptocurrency)AS 'TheLowerLowerPricePerUnit'     ,MAX(volume24hTwd)AS  '24hTwdVolume' ,MAX(TWD_Market_Cap) AS 'UsdMarkCap' from Cryptocurrency WHERE   CONVERT(varchar,InformationDate,120) LIKE '%2022-05-16%' and symbol_of_cryptocurrency =@currencyName group by symbol_of_cryptocurrency 
